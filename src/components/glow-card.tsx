...
export function GlowCard({
  ...
  const initializeParticles = () => {
    if (particlesInitializedRef.current || !cardRef.current || !enableParticles) return;
  ...
  return (
    <div
      ref={cardRef}
      className={`glow-card relative overflow-hidden transition-all duration-300 ${
...
export function GlobalSpotlight({
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      isInsideSectionRef.current = mouseInside;
    ...
    const handleMouseLeave = () => {
      ...
      gridRef.current?.querySelectorAll(".glow-card").forEach((card) => {
        card.style.setProperty("--glow-intensity", "0");
      });
